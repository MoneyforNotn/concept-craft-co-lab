...
const handler = async (req: Request): Promise<Response> => {
  ...
  try {
    const { playerId, title, message }: ScheduledNotificationRequest = await req.json();

    console.log('Sending test notification:', { playerId, title, message });
    ...
    // Send notification immediately (OneSignal free tier doesn't support delayed sending)
